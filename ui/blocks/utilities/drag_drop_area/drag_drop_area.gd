	...
	if button_event.double_click:
		...
		if button_event.button_index == MOUSE_BUTTON_LEFT:
			...
			if parent_block and parent_block.can_delete:
				...
				_context_menu.add_icon_item(EditorInterface.get_editor_theme().get_icon("Duplicate", "EditorIcons"), tr("Duplicate"), MENU_DUPLICATE)
				_context_menu.add_icon_item(EditorInterface.get_editor_theme().get_icon("Remove", "EditorIcons"), tr("Delete"), MENU_DELETE)
...
func _menu_pressed(id: int, _context_menu: PopupMenu):
	match id:
		MENU_DUPLICATE:
