...
func _get_format_string() -> String:
	...
	if definition.property_name and TranslationServer.has_method(&"get_or_add_domain"):
		...
		return _format_property_message(definition.display_template, translated_property.to_lower())

	var variable_name := _get_variable_name()
	if variable_name:
		return _format_variable_message(definition.display_template, variable_name)

	return tr(definition.display_template)


func _format_property_message(message: String, translated_property: String) -> String:
	if message in ["Set the %s property", "Change the %s property", "The %s property"]:
		return tr(message) % translated_property

	if message == "set %%s to {value: %s}" or message.begins_with("set %s to {value: "):
		var type_string := _get_input_type_string("value")
		return (tr("set %%s to {value: %s}") % type_string) % translated_property

	if message == "change %%s by {value: %s}" or message.begins_with("change %s by {value: "):
		var type_string := _get_input_type_string("value")
		return (tr("change %%s by {value: %s}") % type_string) % translated_property

	return tr(message) % translated_property


func _format_variable_message(message: String, variable_name: String) -> String:
	if message in ["Set the %s variable", "The %s variable"]:
		return tr(message) % variable_name

	if message == "set %s to {value: %s}" or message.begins_with("set %s to {value: "):
		var type_string := _get_input_type_string("value")
		return tr("set %s to {value: %s}") % [variable_name, type_string]

	return tr(message)


func _get_variable_name() -> String:
	var block_name := String(definition.name)
	var setter_prefix := String(BlockDefinition.VARIABLE_SETTER_NAME_FORMAT).trim_suffix("%s")
	var getter_prefix := String(BlockDefinition.VARIABLE_GETTER_NAME_FORMAT).trim_suffix("%s")
	if block_name.begins_with(setter_prefix):
		return block_name.trim_prefix(setter_prefix)
	if block_name.begins_with(getter_prefix):
		return block_name.trim_prefix(getter_prefix)
	return ""


func _get_input_type_string(parameter_name: String) -> String:
	for item in BlockDefinition.parse_display_template(definition.display_template):
		if item.has("in_parameter"):
			var parameter: Dictionary = item["in_parameter"]
			if parameter["name"] == parameter_name:
				return _Types.VARIANT_TYPE_TO_STRING[parameter["type"]]
	return _Types.VARIANT_TYPE_TO_STRING[TYPE_NIL]
...
func confirm_delete():
	...
	if num_blocks > 1:
		dialog.dialog_text = tr("Delete %d blocks?") % num_blocks
	else:
		dialog.dialog_text = tr("Delete block?")
...
func _get_tooltip(at_position: Vector2) -> String:
	...
	var description_tx := _get_description()
...
func _get_description() -> String:
	if definition.property_name and TranslationServer.has_method(&"get_or_add_domain"):
		var domain: TranslationDomain = TranslationServer.get_or_add_domain("godot.properties")
		var translated_property: String = domain.translate(definition.property_name.capitalize())
		return _format_property_message(definition.description, translated_property.to_lower())

	var variable_name := _get_variable_name()
	if variable_name:
		return _format_variable_message(definition.description, variable_name)

	return tr(definition.description)
