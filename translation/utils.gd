...
## Locale aliases that should use the same translation resource.
...
static func load_translations():
	...
	for name in locale_dir.get_files():
		...
		_add_translation_with_aliases(domain, po, po_path)


static func _add_translation_with_aliases(domain, translation: Translation, source_path: String):
	print_verbose("Adding %s %s translations from %s" % [DOMAIN, translation.locale, source_path])
	domain.add_translation(translation)

	for alias in LOCALE_ALIASES.get(translation.locale, []):
		var alias_translation := translation.duplicate()
		alias_translation.locale = alias
		print_verbose("Adding %s %s alias translations from %s" % [DOMAIN, alias, source_path])
		domain.add_translation(alias_translation)
